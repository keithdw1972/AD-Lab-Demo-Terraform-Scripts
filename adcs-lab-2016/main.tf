...
# public ip - non domain
#resource "azurerm_public_ip" "nd_public_ip" {
#  name = "${var.resource_prefix}-ND-PublicIP"
#  location            = azurerm_resource_group.windows_rg.location
#  resource_group_name = azurerm_resource_group.windows_rg.name
#  allocation_method   = "Dynamic"
#  tags = var.tags
...
# network interface - non domain
resource "azurerm_network_interface" "nd_nic" {
  name = "${var.resource_prefix}-ND-NIC"
  location            = azurerm_resource_group.windows_rg.location
  resource_group_name = azurerm_resource_group.windows_rg.name
  tags = var.tags
  ...
  ip_configuration {
    ...
    #private_ip_address_allocation = "Dynamic"
    private_ip_address_allocation = "static"
    private_ip_address            = cidrhost(var.node_address_prefix, 40)
  #  public_ip_address_id          = azurerm_public_ip.nd_public_ip.id
  }
}
...
#VM object for the Non Domain Server
resource "azurerm_windows_virtual_machine" "windows_vm_nondomain" {
  name  = "${var.resource_prefix}-ND"
  location              = azurerm_resource_group.windows_rg.location
  resource_group_name   = azurerm_resource_group.windows_rg.name
  network_interface_ids = [azurerm_network_interface.nd_nic.id]
  size                  = var.vmsize
  admin_username        = var.adminuser
  admin_password        = var.adminpassword
